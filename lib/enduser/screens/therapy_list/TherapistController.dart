...
class TherapistController extends BaseController {
  ...
  final RxString sortBy =
      ''.obs; // Only set when user selects a sort option - made observable
  int page = 0;
  int limit = 20;
  ...
  void _receiveAndLogArguments() {
    ...
    if (arguments != null && arguments is Map<String, dynamic>) {
      ...
      if (arguments['sub_service_id'] != null) {
        List<dynamic> subServiceIds =
            arguments['sub_service_id'] as List<dynamic>;
  ...
  void callProfessionalsListAPI() {
    Map<String, dynamic> toJson() {
          ...
          try {
            ...
            if (serviceIdsValue is List) {
              ...
              List<String> serviceIds =
                  serviceIdsValue.map((id) => id.toString()).toList();
          ...
          try {
            ...
            if (subServiceIdsValue is List) {
              ...
              List<String> subServiceIds =
                  subServiceIdsValue.map((id) => id.toString()).toList();
      ...
      if (selectedAvailabilityFilter.value.isNotEmpty) {
        String availabilityApiFormat =
            _convertAvailabilityToApiFormat(selectedAvailabilityFilter.value);
    ...
    var service =
        _repository.sendPostApiRequest(toJson, professionals_list, true);
    callDataService(
      service,
    ...
    try {
      ...
      if (success == true && responseData['data'] != null) {
            ...
            final therapist = Therapist(
              ...
              packages: (json['packages'] as List?)?.map((pkg) {
                    return ServicePackage(
                      ...
                      service_format_date:
                          pkg['service_format_date']?.toString(),
                    );
                  }).toList() ??
                  [],
            );
        ...
        if (filteredTherapists.isNotEmpty) {
          ...
          final analyticsItems = filteredTherapists
              .map(
                (therapist) => AnalyticsService.instance.buildItem(
                  itemId: therapist.id.isNotEmpty ? therapist.id : '',
                  itemName: therapist.specialty.isNotEmpty
                      ? therapist.specialty
                      : (therapist.name.isNotEmpty ? therapist.name : ''),
                  itemCategory: category,
                  itemCategory2: '',
                  itemVariant: therapist.specialty.isNotEmpty
                      ? therapist.specialty
                      : category,
                  itemBrand: therapist.services.isNotEmpty
                      ? therapist.services.first
                      : (therapist.specialty.isNotEmpty
                          ? therapist.specialty
                          : category),
          ...
          final subTypeLabel =
              (label != null && label!.isNotEmpty) ? label! : category;
          final subTypeIdValue = (subTypeId != null && subTypeId!.isNotEmpty)
              ? subTypeId!
              : category;
  ...
  void applyFilters(Map<String, dynamic> filterData) {
    ...
    selectedAvailabilityFilter.value =
        filterData['availability'] as String? ?? '';
  ...
  Future<void> _handleSaveUnsaveSuccess(
      dynamic baseResponse, String therapistId) async {
    try {
      Map<String, dynamic> responseData;
      if (baseResponse != null && baseResponse.data != null) {
        responseData = baseResponse.data is Map<String, dynamic>
      ...
      if (success == true) {
        ...
        final therapistIndex =
            therapists.indexWhere((t) => t.id == therapistId);
        if (therapistIndex != -1) {
          final therapist = therapists[therapistIndex];
